...
export type StageSpawnPoint = {
  ...
  readonly x: number;
  readonly y: number;
  readonly enemyType?: "soldier" | "mauler" | "duelist" | "shield-guard" | "crossbow";
};
...
export function validateStageConfig(config: StageConfig): StageConfig {
  ...
  for (const point of config.spawnPoints) {
    ...
    if (point.enemyType && !["soldier", "mauler", "duelist", "shield-guard", "crossbow"].includes(point.enemyType)) throw new Error(`Unknown enemy type: ${point.enemyType}`);
