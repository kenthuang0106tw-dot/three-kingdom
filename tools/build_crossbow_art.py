...
BASE_SOURCE = "crossbow-source-transparent.png"
EXTRA_SOURCE = "crossbow-extra-source-transparent.png"
BASE_SCALE = 0.71
EXTRA_SCALE = 0.44

FRAMES = (
    SourceFrame("idle-0", BASE_SOURCE, (68, 32, 315, 329), BASE_SCALE),
    SourceFrame("idle-1", BASE_SOURCE, (357, 32, 592, 329), BASE_SCALE),
    SourceFrame("walk-0", BASE_SOURCE, (647, 37, 886, 329), BASE_SCALE),
    SourceFrame("walk-1", BASE_SOURCE, (932, 37, 1178, 329), BASE_SCALE),
    SourceFrame("walk-2", BASE_SOURCE, (1218, 43, 1478, 330), BASE_SCALE),
    SourceFrame("walk-3", BASE_SOURCE, (51, 373, 296, 658), BASE_SCALE),
    SourceFrame("fire-0", BASE_SOURCE, (344, 365, 602, 657), BASE_SCALE),
    SourceFrame("fire-1", BASE_SOURCE, (621, 368, 896, 657), BASE_SCALE),
    SourceFrame("fire-2", BASE_SOURCE, (927, 370, 1162, 657), BASE_SCALE),
    SourceFrame("hurt-0", BASE_SOURCE, (1216, 388, 1425, 657), BASE_SCALE),
    SourceFrame("hurt-1", BASE_SOURCE, (49, 701, 267, 968), BASE_SCALE),
    SourceFrame("dead-0", BASE_SOURCE, (332, 720, 573, 968), BASE_SCALE),
    SourceFrame("dead-1", BASE_SOURCE, (592, 754, 846, 968), BASE_SCALE),
    SourceFrame("dead-2", BASE_SOURCE, (884, 719, 1154, 968), BASE_SCALE),
    SourceFrame("dead-3", BASE_SOURCE, (1188, 863, 1509, 988), BASE_SCALE),
    SourceFrame("aim-0", EXTRA_SOURCE, (36, 137, 432, 612), EXTRA_SCALE),
    SourceFrame("aim-1", EXTRA_SOURCE, (454, 147, 902, 612), EXTRA_SCALE),
    SourceFrame("locked-0", EXTRA_SOURCE, (898, 153, 1374, 612), EXTRA_SCALE),
    SourceFrame("reload-0", EXTRA_SOURCE, (1380, 149, 1755, 612), EXTRA_SCALE),
    SourceFrame("reload-1", EXTRA_SOURCE, (1761, 144, 2166, 612), EXTRA_SCALE),
)

ANIMATIONS = {
    "idle": ["idle-0", "idle-1"],
    "walk": ["walk-0", "walk-1", "walk-2", "walk-3"],
    "fire": ["fire-0", "fire-1", "fire-2"],
    "hurt": ["hurt-0", "hurt-1"],
...
def build() -> tuple[Image.Image, dict]:
    ...
    for index, frame in enumerate(FRAMES):
        ...
        if pose.width > CELL - 16 or pose.height >= FEET_Y:
    ...
    sheet.save(ART / "crossbow.png")
    atlas = {
        "frames": atlas_frames,
        "meta": {
            "app": "Three Kingdom Crossbow production builder",
            "version": "1.0",
            "image": "crossbow.png",
            "format": "RGBA8888",
    ...
    metadata = {
        ...
        "displayScale": DISPLAY_SCALE,
        "sourceFacing": 1,
        "targetLogicalIdleHeight": 210,
        "logicalIdleHeight": round(metadata_frames[0]["runtimeAlphaBounds"]["height"] * DISPLAY_SCALE, 2),
        "animations": ANIMATIONS,
        "provenance": {
            "original": True,
            "identityGate": "crossbow-idle-gate-source.png",
            "sources": ["crossbow-source.png", "crossbow-extra-source.png"],
            "sourceLayout": "measured-five-by-three-plus-five-by-one",
            "processingTool": "tools/build_crossbow_art.py",
            "processingRevision": "1.0",
            "generatedBy": "built-in image generation, ER.6 Crossbow Production-Art Replacement",
        },
        "frames": metadata_frames,
    }
    (ART / "crossbow.atlas.json").write_text(json.dumps(atlas, indent=2), encoding="utf-8")
    (ART / "crossbow.metadata.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")
...
def build_qa(sheet: Image.Image, frames: list[dict]) -> None:
    ...
    debug.save(ART / "crossbow-debug.png")
    ...
    for names in ANIMATIONS.values():
        ...
        for name in names:
            ...
            previous = current
    onion.save(ART / "crossbow-onion.png")
    ...
    silhouette.save(ART / "crossbow-silhouette-25.png")
    ...
    color_reference = color_reference.crop((1380, 130, 1740, 760))
    silhouette_reference = silhouette_reference.crop((1380, 160, 1740, 760))
    ...
    review.alpha_composite(runtime, (1010, 510 - CELL))
    ...
    review.save(REVIEWS / "crossbow-er6-idle-gate.png")
    debug.save(REVIEWS / "crossbow-er6-review.png")
