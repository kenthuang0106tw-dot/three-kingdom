...
- Audio：`public/audio/sfx/` contains ten original project-owned mono 16-bit PCM WAV cues generated by `tools/build_combat_ui_sfx.mjs`; `combat-ui-sfx.metadata.json` records per-file duration, encoding, SHA-256, source, processing, license, and description. Runtime loading is manifest-owned; no third-party sample is used.
...
- **M7.3:** 原創／授權 Stage/Boss BGM，必須保留 source、license/provenance、runtime encoding 與 transition mapping。
