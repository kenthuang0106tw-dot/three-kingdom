...
M7 / Tasks 7.1–7.2 已建立單一 Phaser Audio manager，並接入十個原創 Combat/UI SFX、immutable event mapping、multi-target hit coalescing、pause/visibility、unlock 與 Scene restart cleanup。

The next single task is M7 / Task 7.3: add original Stage/Boss music through the accepted Audio manager.

下一個 Task 只加入原創／授權的 Stage/Boss BGM 與無重播、無斷裂的 transition mapping；不修改 Combat/UI SFX、戰鬥或已凍結美術。
